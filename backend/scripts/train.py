"""
Retrain the v2 service-prediction model on the corrected dataset (Phase 3).

Same modeling approach as `training/train_v2.py` (ColumnTransformer with
StandardScaler for numeric features and OneHotEncoder for categorical ones,
MultiOutputRegressor over a small candidate set, winner picked by mean MAE
on a held-out split, then refit on the full corrected dataset before
persisting) - kept identical so this is a genuine apples-to-apples retrain
on corrected data, not a parallel modeling approach.
...
def train() -> dict:
    ...
    winner_pipeline.fit(X, y)   # retrain on full corrected data before saving

    joblib.dump(winner_pipeline, MODEL_PATH)
    print(f"\nWinner: {winner_name} -> {MODEL_PATH}")
    print(f"Held-out MAE: days={winner_metrics['days_until_service']}, "
          f"kms={winner_metrics['kms_until_service']}, mean={winner_metrics['mean_mae']}")
